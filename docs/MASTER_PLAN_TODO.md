...
- [x] **index.html — HOME v2: complete section redesign** (replaces v1 copy-swap). Old body (st-spaces/partners, story, stats, orbit, split, foundations, approach/FAQ, marquee, featured, banner CTA) fully replaced with 8 new `hm-*` sections per MASTER §6.1 + CONTENT-REFRESH: ① hm-statement ② hm-pillars (4 pillars → services) ③ hm-costlock ④ hm-gates (7 gates, Gate 4 wood-locked) ⑤ hm-tiles (4 intent entries) ⑥ hm-proof (locked numbers) ⑦ hm-quotes ⑧ hm-cta (WhatsApp + NAP). Hero 3-slide kept with v1 copy. FAQPage JSON-LD removed (FAQ section gone); Organization schema kept. home.css pruned 2103→933 lines (dead section + inner-page styles); `?v=4`; lx.css/studio.css links dropped from index. Testimonials: 3 placeholder quotes from brief with `[CONFIRM: replace with real client quotes before launch]` — still blocked on real quotes.
