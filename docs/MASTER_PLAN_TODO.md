...
- [x] **Insights single-post (article) template polish pass — QUEUED per owner decision 2026-09-20** (richer article template: typography, article header/meta, reading structure)
- ⏳ **Next:** insights article-template polish → process · contact · start-your-project · faq (conversion core) → locations ×13 → client-stories · woodex-craft · careers → architecture/renovation/rooms expansion (~28)
...
- [x] **SERVICES FAMILY — REBUILT (hub + 20 pages, 2026-09-20).** Owner decisions locked: home kept as-is · pricing HELD · founder/warranty stay [CONFIRM]. Single-service template (8 named blocks): page-hero+meta · covers · who-for · process · proof-band · faq-accordion (3/page, FAQPage schema) · related-services · unique CTA per page. Hub: page-hero · pillar-grid · service-index (5 groups, business-model priority order) · path-band + Cost-Lock · cta. css/services.css?v=1; visualization bridges to 3d-studio; Wellstar named on pharmacy only. Verified @1440/390. Reality (superseded): the uneditable header mega menu links 20 `services/<slug>.html` pages (not the 8-page `/service/` plan) — all 20 URLs stay and get rebuilt on the single-service template. css/services.css?v=1 written; per-service copy digested from the docker briefs (architecture-services · retail/pharmacy/restaurant-fit-out · turnkey-solutions · plan phases 2–4 · sector-* · Residential Interior 7 sub-pages · master-planning · 3d-studio-hub). Pricing stays OUT (fact lock); Service + FAQPage + BreadcrumbList schema with fact-locked provider.
...
- [x] **Projects: hub + single-study template + 6 studies + 2 category pages rebuilt** per docker/project-case-study-template.md (8-block study anatomy; Wellstar named-work band; studies stay labelled studies with [CONFIRM: real project data]; Service schema with fact-locked provider; css/projects.css?v=1). [ ] locations.html hub remains.
